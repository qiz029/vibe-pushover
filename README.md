...
```sh
curl -fsSL https://github.com/qiz029/vibe-pushover/releases/download/v0.20.0/install.sh | \
  VIBE_PUSHOVER_VERSION=v0.20.0 VIBE_PUSHOVER_INSTALL_DIR="$HOME/bin" sh
```
...
- If a hook payload supplies a Unix or RFC3339 event timestamp, it is forwarded through Pushover's [`timestamp` parameter](https://pushover.net/api#customizing) so concurrently delivered or delayed notifications are ordered by when the agent event actually happened, rather than when the API received it.
