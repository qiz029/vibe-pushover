...
func TestDetectedAgentsFindsInstalledCLIsBeforeFirstRun(t *testing.T) {
	...
	binDir := t.TempDir()
	for _, name := range []string{"ccr", "cline", "codebuddy", "gptme", "junie", "kiro-cli", "kilo", "omp", "openhands", "opencode", "qodercli", "tabnine"} {
	...
	want := []string{"claude-router", "cline", "codebuddy", "gptme", "junie", "kiro", "kilo", "omp", "openhands", "opencode", "qoder", "tabnine"}
	if !reflect.DeepEqual(got, want) {
...
func TestDefaultPathsForAdditionalAgents(t *testing.T) {
	...
	tests := map[string]string{
		...
		"gptme":       filepath.Join(home, ".config", "gptme", "config.toml"),
...
func TestDefaultPathGptmeUsesDocumentedUserConfig(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("XDG_CONFIG_HOME", filepath.Join(home, "ignored-xdg"))

	got, err := hooks.DefaultPath("gptme")
	if err != nil {
		t.Fatalf("DefaultPath() error = %v", err)
	}
	want := filepath.Join(home, ".config", "gptme", "config.toml")
	if got != want {
		t.Fatalf("DefaultPath() = %q, want %q", got, want)
	}
}

func TestInstallGptmePluginPreservesConfigAndIsIdempotent(t *testing.T) {
	t.Parallel()

	path := filepath.Join(t.TempDir(), ".config", "gptme", "config.toml")
	original := "# keep this comment\n[chat]\nmodel = \"openai/gpt-5\"\n"
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	...
	changed, err := hooks.Install("gptme", path, "/opt/bin/vibe-pushover", "/tmp/pushover config.json")
	if err != nil {
		t.Fatalf("Install() error = %v", err)
	}
	if !changed {
	...
	if !bytes.Contains(data, []byte(original)) {
		t.Fatalf("gptme config did not preserve unrelated content:\n%s", data)
	}
	pluginRoot := filepath.Join(filepath.Dir(path), "plugins")
	var parsed struct {
		Plugins struct {
	...
	if !reflect.DeepEqual(parsed.Plugins.Paths, []string{pluginRoot}) {
		t.Fatalf("plugins.paths = %#v, want %#v", parsed.Plugins.Paths, []string{pluginRoot})
	}

	initPath := filepath.Join(pluginRoot, "vibe_pushover", "__init__.py")
	hookPath := filepath.Join(pluginRoot, "vibe_pushover", "hooks", "notify.py")
	for _, generatedPath := range []string{initPath, hookPath} {
		info, err := os.Stat(generatedPath)
		if err != nil {
			t.Fatalf("Stat(%q) error = %v", generatedPath, err)
		}
		if runtime.GOOS != "windows" && info.Mode().Perm() != 0o600 {
			t.Fatalf("mode(%q) = %o, want 600", generatedPath, info.Mode().Perm())
		}
	}
	...
	for _, want := range []string{
		"# Generated by vibe-pushover.",
		`BINARY = "/opt/bin/vibe-pushover"`,
		`CONFIG_PATH = "/tmp/pushover config.json"`,
		`HookType.TURN_POST`,
		`async_mode=True`,
		`HookType.TOOL_CONFIRM`,
		`priority=1`,
		`observer_index + 1`,
		`next_hooks[0].name != "cli_confirm"`,
		`is_auto_confirm_active()`,
		`except Exception:`,
		`must never affect gptme's approval decision`,
		`"last_assistant_message"`,
		`os.environ.get("GPTME_SUBAGENT_AGENT_ID")`,
		`"tool_name"`,
		`"approval-required"`,
	} {
		if !bytes.Contains(hook, []byte(want)) {
			t.Fatalf("generated gptme hook does not contain %q:\n%s", want, hook)
		}
	}

	beforeConfig := append([]byte(nil), data...)
	beforeHook := append([]byte(nil), hook...)
	changed, err = hooks.Install("gptme", path, "/opt/bin/vibe-pushover", "/tmp/pushover config.json")
	if err != nil {
		t.Fatalf("second Install() error = %v", err)
	}
	...
	afterHook, _ := os.ReadFile(hookPath)
	if !bytes.Equal(afterConfig, beforeConfig) || !bytes.Equal(afterHook, beforeHook) {
		t.Fatal("second Install() rewrote gptme files")
	}
}

func TestInstallGptmePluginMergesPathsAndEnabledAllowlist(t *testing.T) {
	t.Parallel()

	path := filepath.Join(t.TempDir(), "gptme", "config.toml")
	original := `# plugin settings
[plugins]
paths = [
  "/tmp/existing", # keep this path comment
]
enabled = ["existing"] # keep allowlist comment
...
`
	...
	if _, err := hooks.Install("gptme", path, "/opt/bin/vibe-pushover", ""); err != nil {
		t.Fatalf("Install() error = %v", err)
	}
	data, err := os.ReadFile(path)
	if err != nil {
	...
	if !bytes.Contains(data, []byte("# plugin settings")) || !bytes.Contains(data, []byte("# keep this path comment")) || !bytes.Contains(data, []byte("# keep allowlist comment")) || !bytes.Contains(data, []byte("[env]\nKEEP = \"yes\"")) {
		t.Fatalf("comments or unrelated tables were not preserved:\n%s", data)
	}
	var parsed struct {
		Plugins struct {
			Paths   []string `toml:"paths"`
	...
	wantPaths := []string{"/tmp/existing", filepath.Join(filepath.Dir(path), "plugins")}
	if !reflect.DeepEqual(parsed.Plugins.Paths, wantPaths) {
		t.Fatalf("plugins.paths = %#v, want %#v", parsed.Plugins.Paths, wantPaths)
	}
	if !reflect.DeepEqual(parsed.Plugins.Enabled, []string{"existing", "vibe_pushover"}) {
		t.Fatalf("plugins.enabled = %#v", parsed.Plugins.Enabled)
	}
}

func TestInstallGptmePluginLeavesEmptyEnabledAllowlistOpen(t *testing.T) {
	t.Parallel()
	...
	if len(parsed.Plugins.Enabled) != 0 || !bytes.Contains(data, []byte("# all discovered plugins")) {
		t.Fatalf("empty allowlist semantics changed:\n%s", data)
	}
}

func TestInstallGptmePluginMergesDottedPluginKeys(t *testing.T) {
	t.Parallel()

	path := filepath.Join(t.TempDir(), "gptme", "config.toml")
	original := "# dotted form\nplugins.paths = [\"/tmp/existing\"]\nplugins.enabled = [\"existing\"]\n"
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	if err := os.WriteFile(path, []byte(original), 0o600); err != nil {
	...
	if err := toml.Unmarshal(data, &parsed); err != nil {
		t.Fatalf("generated dotted config is invalid TOML: %v\n%s", err, data)
	}
	if !bytes.Contains(data, []byte("# dotted form")) || !reflect.DeepEqual(parsed.Plugins.Enabled, []string{"existing", "vibe_pushover"}) || !containsTestString(parsed.Plugins.Paths, filepath.Join(filepath.Dir(path), "plugins")) {
		t.Fatalf("dotted plugin config was not merged safely:\n%s", data)
	}
}

func TestInstallGptmePluginMergesInlinePluginTable(t *testing.T) {
	t.Parallel()

	path := filepath.Join(t.TempDir(), "gptme", "config.toml")
	original := "# inline form\nplugins = { enabled = [\"existing\"], keep = \"yes\" } # keep inline comment\n"
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	if err := os.WriteFile(path, []byte(original), 0o600); err != nil {
	...
	var parsed map[string]any
	if err := toml.Unmarshal(data, &parsed); err != nil {
		t.Fatalf("generated inline config is invalid TOML: %v\n%s", err, data)
	}
	plugins, ok := parsed["plugins"].(map[string]any)
	if !ok {
		t.Fatalf("plugins = %#v", parsed["plugins"])
	}
	paths, _ := plugins["paths"].([]any)
	enabled, _ := plugins["enabled"].([]any)
	if plugins["keep"] != "yes" || !bytes.Contains(data, []byte("# keep inline comment")) || len(paths) != 1 || len(enabled) != 2 || enabled[1] != "vibe_pushover" {
		t.Fatalf("inline plugin config was not merged safely:\n%s", data)
	}
}
...
func TestInstallGptmePluginRefusesUnownedFile(t *testing.T) {
	t.Parallel()

	path := filepath.Join(t.TempDir(), "gptme", "config.toml")
	hookPath := filepath.Join(filepath.Dir(path), "plugins", "vibe_pushover", "hooks", "notify.py")
	if err := os.MkdirAll(filepath.Dir(hookPath), 0o700); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	original := []byte("# user-owned plugin\n")
	if err := os.WriteFile(hookPath, original, 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	if _, err := hooks.Install("gptme", path, "/opt/bin/vibe-pushover", ""); err == nil || !strings.Contains(err.Error(), "not owned by vibe-pushover") {
		t.Fatalf("Install() error = %v, want ownership refusal", err)
	}
	...
	if !bytes.Equal(after, original) {
		t.Fatal("Install() changed unowned gptme plugin")
	}
	if _, err := os.Stat(path); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("config was changed after ownership refusal: %v", err)
	}
}

func TestInstallGptmePluginPreservesConfigSymlink(t *testing.T) {
	t.Parallel()

	dir := t.TempDir()
	target := filepath.Join(dir, "dotfiles", "gptme.toml")
	if err := os.MkdirAll(filepath.Dir(target), 0o700); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	if err := os.WriteFile(target, []byte("[chat]\nmodel = \"openai/gpt-5\"\n"), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	path := filepath.Join(dir, ".config", "gptme", "config.toml")
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
	...
	if info.Mode()&os.ModeSymlink == 0 {
		t.Fatal("Install() replaced the gptme config symlink")
	}
	data, err := os.ReadFile(target)
	if err != nil {
		t.Fatalf("ReadFile(target) error = %v", err)
	...
	pluginRoot := filepath.Join(filepath.Dir(path), "plugins")
	if !bytes.Contains(data, []byte(strconv.Quote(pluginRoot))) {
		t.Fatalf("symlink target does not reference plugin beside config link:\n%s", data)
	}
	if _, err := os.Stat(filepath.Join(pluginRoot, "vibe_pushover", "hooks", "notify.py")); err != nil {
		t.Fatalf("plugin was not installed beside config link: %v", err)
	}
}

func TestInstallGptmePluginRejectsInvalidPluginTypesBeforeWriting(t *testing.T) {
	t.Parallel()
	...
	if _, err := hooks.Install("gptme", path, "/opt/bin/vibe-pushover", ""); err == nil || !strings.Contains(err.Error(), "parse gptme config") {
		t.Fatalf("Install() error = %v, want plugin type error", err)
	}
	pluginPath := filepath.Join(filepath.Dir(path), "plugins", "vibe_pushover", "hooks", "notify.py")
	if _, err := os.Stat(pluginPath); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("plugin was written despite invalid config: %v", err)
	}
}
